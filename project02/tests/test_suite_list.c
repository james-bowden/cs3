...
void test_list_size1() {
    ...
    *v = VEC_ZERO;
    ...
    // Remove
    assert((vector_t *)list_remove(l) == v);
    free(v);
    assert(list_size(l) == 0);
    // Add again
    v = malloc(sizeof(*v));
...
void test_list_large_add_remove() {
    ...
    for (size_t i = 1; i <= LARGE_SIZE; i++) {
        ...
        vector_t *v = (vector_t *)list_remove(l);
...
void test_out_of_bounds_access() {
    ...
    list_free(access->list, vec_free);
    free(access);
}
...
void test_empty_remove() {
    ...
    for (size_t i = 0; i < size; i++) {
        vector_t *list_v = (vector_t *)list_remove(l);
...
void test_resize(){
  ...
  list_t *l = list_init(RESIZE_START_SIZE);
