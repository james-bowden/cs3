...
void test_list_size0() {
    list_t *l = list_init(0, vec_free);
    assert(list_size(l) == 0);
    list_free(l);
}

void test_list_size1() {
    list_t *l = list_init(1, vec_free);
    assert(list_size(l) == 0);
    // Add
...
void test_list_small() {
    ...
    *v = VEC_ZERO;
    ...
    // Remove some
    v = (vector_t *)list_remove(l, list_size(l) - 1);
...
void test_list_large_get_set() {
    list_t *l = list_init(LARGE_SIZE, vec_free);
    // Add to capacity
...
void test_list_large_add_remove() {
    ...
    for (size_t i = 1; i <= LARGE_SIZE; i++) {
        ...
        vector_t *v = (vector_t *)list_remove(l, list_size(l) - 1);
...
void test_out_of_bounds_access() {
    ...
    access->list = list_init(max_size, vec_free);
...
void test_empty_remove() {
    ...
    list_t *l = list_init(size, vec_free);
...
#define RESIZE_START_SIZE 10

void test_resize(){
  // accounts for 5 resizing iterations
  int end_size = 10 * 2 * 2 * 2 * 2 * 2;
  list_t *l = list_init(RESIZE_START_SIZE, vec_free);
  // Add one over capacity
  for (size_t i = 0; i <= RESIZE_START_SIZE; i++) {
      vector_t *v = malloc(sizeof(*v));
      v->x = v->y = i;
      list_add(l, v);
  }
  // Check
  assert(list_size(l) == RESIZE_START_SIZE + 1);
  assert(list_capacity(l) == 2 * RESIZE_START_SIZE);
  for (size_t i = 0; i <= RESIZE_START_SIZE; i++) {
      assert(vec_equal(*(vector_t *)list_get(l, i), (vector_t) {i, i}));
  }
  // add some more, should just resize and be fine
  for (size_t i = RESIZE_START_SIZE + 1; i < (end_size / 2) + 1; i++) {
      vector_t *v = malloc(sizeof(*v));
      v->x = v->y = i;
      list_add(l, v);
  }
  //check that is fine
  assert(list_capacity(l) == end_size);
  assert(list_size(l) == (end_size / 2) + 1);
  // Set every 100th value
  for (size_t i = 0; i < (end_size / 2) + 1; i += 100) {
      vector_t *v = (vector_t *)list_get(l, i);
      v->x = v->y = i * 10;
  }
  // Check all values again
  for (size_t i = 0; i < (end_size / 2) + 1; i++) {
      assert(vec_equal(
          *(vector_t *)list_get(l, i),
          i % 100 == 0 ? (vector_t) {i * 10, i * 10} : (vector_t) {i, i}
      ));
